...
def replace_text_in_files(target, old_text, new_text, file_extension=None, is_file=False, specific_filename=None):
    ...
    if is_file:
        ...
        files = [os.path.join(foldername, filename) for foldername, _, filenames in os.walk(target) for filename in filenames if (not file_extension or filename.endswith(file_extension)) and (not specific_filename or filename == specific_filename)]
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--name', help='The specific filename to target (optional)')
    
    args = parser.parse_args()
    
    if args.file:
        replace_text_in_files(args.file, args.old_text, args.new_text, args.file_extension, is_file=True, specific_filename=args.name)
    else:
        replace_text_in_files(args.directory, args.old_text, args.new_text, args.file_extension, is_file=False, specific_filename=args.name)
